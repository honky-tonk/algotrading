...
*/
func (ema EMA_Indicator) Get_Indicator(s asset.Stocks) ([]Price, error) {
	if ema.Smoothing == 0 {
	...
	ema.Indicator_Value = append(ema.Indicator_Value, Price{P: init_ema_price, T: s.Prices[ema.Period-1].T})
	...
	for i := ema.Period; i < len(s.Prices); i++ {
		...
		ema.Indicator_Value = append(ema.Indicator_Value, Price{P: today_price*Multipler + (yeasterday_ema * (1 - Multipler)), T: s.Prices[i].T})
